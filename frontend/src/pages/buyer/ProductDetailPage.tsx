...
import { publicProductAPI, PublicProductDetail, ProductImage } from '../../services/api';
...
const S = {
  ...
  thumb: (active: boolean) => ({
    width: '70px', height: '70px', borderRadius: '6px', objectFit: 'cover' as const, border: active ? '2px solid #7c3aed' : '1px solid #e2e8f0', cursor: 'pointer'
  }),
  infoCard: { display: 'flex', flexDirection: 'column' as const, gap: '1.25rem' },
  brand: { fontSize: '0.85rem', fontWeight: '700', textTransform: 'uppercase' as const, color: '#7c3aed', letterSpacing: '0.05em' },
  ...
  chip: (selected: boolean) => ({
    padding: '0.5rem 1rem', borderRadius: '8px', border: selected ? '2px solid #7c3aed' : '1px solid #e2e8f0', background: selected ? '#f5f3ff' : '#fff', color: selected ? '#7c3aed' : '#475569', fontWeight: '600', cursor: 'pointer', fontSize: '0.85rem'
  }),
...
export const ProductDetailPage: React.FC = () => {
  ...
  if (loading) return <div style={S.container}>Loading product details...</div>;
  if (!product) {
    return (
